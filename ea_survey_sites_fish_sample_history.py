...
class EASurveySitesFishHistory(EAEcologyAndFishData):
    ...
    def get_data(self):
        ...
        EASurveySitesFishHistory.fish_datafile = (
            super().download_and_extract(EASurveySitesFishHistory.fish_zipfile)
            + EASurveySitesFishHistory.fish_filename
        )

    def process_data(self):
        # Construct dataframe and write result to base class attribute 'dataframe'
        self.dataframe = self.construct_df(EASurveySitesFishHistory.fish_datafile)
    ...
    def construct_df(file):  # Read csv file to dataframe as required
        ...
        dataframe = pandas.read_csv(
            file, usecols=columns_required, dtype=str, parse_dates=["EVENT_DATE"]
        )

        # Sort on site_id and last_survey then keep only the first row for each site_id
        dataframe = dataframe.sort_values(
            ["SITE_ID", "EVENT_DATE"], ascending=[True, False]
        )
        # Add 'year' column
        dataframe["year"] = ""
        ...
        # Drop any rows with duplicated survey_id  
        dataframe = dataframe.drop_duplicates('SURVEY_ID', keep='first', ignore_index=True)
        
        # Rename and reorder columns
        renaming = {
            "SITE_ID": "fish_site_id",
            "SITE_NAME": "fish_site_name",
            "SURVEY_ID": "fish_survey_id",
            "EVENT_DATE": "fish_survey_date",
            "SURVEY_METHOD": "survey_method",
            "SURVEY_STRATEGY": "survey_strategy",
    ...
    def append_placeholder(self):
        ...
        """
        placeholder = {
            "fish_site_id": "site_id_placeholder",
            "fish_site_name": "PLACEHOLDER",
            "lat": 0.0,
            "long": 0.0,
            "fish_survey_id": "survey_id_placeholder",
            "survey_method": "method_placeholder",
            "survey_strategy": "strategy_placeholder",
            "fish_survey_date": "2000-01-01",
            "year": "year_palceholder",
            #            'CaBA_Partn': '',
            "CaBA_ID": str(123),
            "CaBA_Catch": ""
            #            'WFD_RBD': ''
        ...
        self.dataframe = pandas.concat(
            [pandas.DataFrame(temp_data), self.dataframe], ignore_index=True
        )
